...
int main( int t_numarg, char **t_arg )
{
	...
	cu_create_alphaimg( l_alphaimg_cuda_img, { 255, 0, 0 }, { 50, 50}, 40, 100 );

	//cv::imshow( "Alpha channel", l_alphaimg_cv_img );

	cu_insertimage( l_chessboard_cuda_img, l_alphaimg_cuda_img, { 0, 0 } );

	cv::imshow( "Šachovnice", l_chessboard_cv_img );

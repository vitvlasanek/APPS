...
__global__ void kernel_draw_circle ( CudaImg t_color_cuda_img, uchar3 t_color , int2 t_position, int t_r , int alpha)
{
	// X,Y coordinates and check image dimensions
	int l_y = blockDim.y * blockIdx.y + threadIdx.y;
	int l_x = blockDim.x * blockIdx.x + threadIdx.x;
	...
	int l_dx = l_x - t_position.x;
	int l_dy = l_y - t_position.y;
	int l_r = sqrtf( l_dx * l_dx + l_dy * l_dy);


	// Store point into image
	t_color_cuda_img.m_p_uchar4[ l_y * t_color_cuda_img.m_size.x + l_x ] =
		{ t_color.x, t_color.y, t_color.z, ( unsigned char ) ( alpha - alpha * (l_r > t_r)) };
}

void cu_create_alphaimg( CudaImg t_color_cuda_img, uchar3 t_color, int2 t_pos, int t_r , int alpha)
{
	cudaError_t l_cerr;
	...
	kernel_draw_circle<<< l_blocks, l_threads >>>( t_color_cuda_img, t_color ,t_pos, t_r, alpha);
